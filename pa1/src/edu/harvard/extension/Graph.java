...
public class Graph implements IGraph {
	...
	public List<Edge> generateGraphDriver(int numVertices, int dimension) {
		if (dimension == 0 || dimension == 2 || dimension == 3 || dimension == 4) {
			return dimension == 0 ? generate0DimensionalGraph(numVertices)
					: generateHigherDimensionalGraph(numVertices, dimension);
		}
		throw new RuntimeException("Dimensions are invalid");
	}

	/**
	 * Generates 0 dimensional graph. Throws out edges beyond what is predicted by
	 * the regression line of the max weight claimed by the spanning tree
	 * 
	 * @param numVertices
	 * @return List<Edge>
	 */
	...
	public List<Edge> generate0DimensionalGraph(int numVertices) {
		...
		// Cut the graph by a constant factor of half by considering only the bottom
		// triangle
	...
	/**
	 * Anything beyond the weight returned here is unlikely to be part of the
	 * spanning tree and ignored
	 * 
	 * @param numVertices
	 * @param dimension
	 * @return weight
	...
	private double throwOutBeyond(int numVertices, int dimension) {
		...
		if (dimension == 2) {
			return (0.01 / (0.0203838847847773737 * numVertices + 4.002965874727))
					+ (numVertices < 64 ? 0.32 : numVertices <= 2048 ? 0.2 : 0.02);
		} else if (dimension == 3) {
			return (0.01 / (0.0203838847847773737 * numVertices + 4.002965874727))
					+ (numVertices <= 128 ? 0.45 : numVertices <= 2048 ? 0.2 : 0.05);
		} else if (dimension == 4) {
			return (0.01 / (0.0203838847847773737 * numVertices + 4.002965874727))
					+ (numVertices <= 128 ? 0.55 : numVertices <= 16384 ? 0.3 : 0.07);
		}

		return (0.01 / (0.0203838847847773737 * numVertices + 4.002965874727)) + (numVertices <= 128 ? 0.28
				: numVertices < 512 ? 0.05
						: numVertices <= 2048 ? 0.07
								: numVertices < 16384 ? 0.005 : numVertices < 16384 ? 0.0009 : 0.0005);
	}
}
